...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Barang Bermasalah</h1>
        <a href="{{ route('issued_good.create') }}" class="btn btn-primary btn-sm shadow-sm">
            <i class="fas fa-plus fa-sm text-white-50"></i> Tambah Barang Bermasalah
        </a>
    </div>
                ...
                <table class="table table-bordered" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $filter_item->detail }}</td>
                            <td>{{ $filter_item->date_issued }}</td>
                            <td>{{ $filter_item->date_fixed }}</td>
                            <td>{{ $filter_item->condition }}</td>
                            <td>
                                @if ($filter_item->condition === 'BERMASALAH')
                                <a href="{{ route('issued_good.edit', $filter_item->id) }}" class="btn btn-info">
                                    <i class="fa fa-pencil-alt"></i>
                                </a>
                                @endif
                                <form action="{{ route('issued_good.destroy', $filter_item->id) }}" method="POST"
                                    class="d-inline">
                                    @csrf
                                    @method('delete')
