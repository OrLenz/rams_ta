...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <a href="{{ route('damaged_good.create') }}" class="btn btn-primary btn-sm shadow-sm">
            <i class="fas fa-plus fa-sm text-white-50"></i> Tambah Barang Rusak
        </a>
    </div>
                ...
                <table class="table table-bordered" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $filter_item->date_damaged }}</td>
                            <td>{{ $filter_item->date_repaired }}</td>
                            <td>{{ $filter_item->condition }}</td>
                            <td>
                                @if ($filter_item->condition === 'RUSAK')
                                <a href="{{ route('damaged_good.edit', $filter_item->id) }}" class="btn btn-info">
                                    <i class="fa fa-pencil-alt"></i>
                                </a>
                                @endif
                                <form action="{{ route('damaged_good.destroy', $filter_item->id) }}" method="POST"
                                    class="d-inline">
                                    @csrf
                                    @method('delete')
