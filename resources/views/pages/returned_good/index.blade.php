...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <a href="{{ route('returned_good.create') }}" class="btn btn-primary btn-sm shadow-sm">
            <i class="fas fa-plus fa-sm text-white-50"></i> Tambah Barang Dikembalikan
        </a>
    </div>
                ...
                <table class="table table-bordered" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $filter_item->date_received }}</td>
                            <td>{{ $filter_item->status }}</td>
                            <td>
                                @if ($filter_item->status === 'DIKEMBALIKAN')
                                <a href="{{ route('returned_good.edit', $filter_item->id) }}" class="btn btn-info">
                                    <i class="fa fa-pencil-alt"></i>
                                </a>
                                @endif
                                <form action="{{ route('returned_good.destroy', $filter_item->id) }}" method="POST"
                                    class="d-inline">
                                    @csrf
                                    @method('delete')
